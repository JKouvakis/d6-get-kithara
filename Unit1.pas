...
procedure getcoockies(Sender: TObject);
var
   i         : Integer;
begin
  Form1.Idhttp1.Get('http://www.kithara.vu/');
  Form1.Memo2.Lines := Form1.Idhttp1.Response.RawHeaders;
    for  i:=0 to Form1.Memo2.Lines.Count do
      begin
      if (Leftstr(Form1.Memo2.Lines.Strings[i],11)='Set-Cookie:') then
        begin
        If (Form1.Edit2.Text = '') then
          Form1.Edit2.Text := MidStr(Form1.Memo2.Lines.Strings[i],13,length(Form1.Memo2.Lines.Strings[i]))
        else
          Form1.Edit4.Text := MidStr(Form1.Memo2.Lines.Strings[i],13,pos(';',Form1.Memo2.Lines.Strings[i])-13);
        end;
      end;
end;
